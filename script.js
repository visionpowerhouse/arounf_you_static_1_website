...
function buildPreviewSrc(base, opts = {}){
  if(!base) return '';
...
function openVideoModal(){
  ...
  if(previewIframe){
    previewIframe.src = buildPreviewSrc(modalBaseSrc, { mute:1 });
  }
  videoModal.style.display = 'flex';
  videoModal.classList.add('open');
  document.body.classList.add('modal-open');
  if(modalIframe){
    modalIframe.src = buildPreviewSrc(modalBaseSrc, { autoplay:1 });
  }
}
...
function closeVideoModal(){
  ...
  if(previewIframe){
    previewIframe.src = buildPreviewSrc(modalBaseSrc);
  }
}
...
if(modalIframe){
  modalIframe.addEventListener('error', ()=>{
    window.open('https://drive.google.com/file/d/1s8RgepREGMlQm60pZdRi5xFSZ8eXL0WF/preview', '_blank');
  }, { once:true });
}
