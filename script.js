...
if(previewImage){
  const mainTrigger = previewImage.closest('[data-preview-trigger]');
  if(mainTrigger){
    mainTrigger.addEventListener('keypress', (e)=>{
      if(e.key === 'Enter' || e.key === ' '){
        e.preventDefault();
        openVideoModal();
      }
...
if(modalIframe){
  modalIframe.addEventListener('error', ()=>{
    window.open(modalBaseSrc || 'https://drive.google.com/file/d/1s8RgepREGMlQm60pZdRi5xFSZ8eXL0WF/preview', '_blank');
  }, { once:true });
}
